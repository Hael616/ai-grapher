...
class ProjectsStore {
  ...
  private loadProjects = async () => {
    ...
    if (error) {
      ...
      this.projects = this.reduceProject([]);
      return;
    }

    this.projects = this.reduceProject(data.length > 0 ? data : mockProjects); // TODO: Remove mock projects
  };
  ...
  public getProject = (
    id: string,
  ): { error?: string; data?: Tables<"projects"> } => {
